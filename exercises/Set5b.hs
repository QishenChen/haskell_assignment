...
valAtRoot (Node a son1 son2) = Just a
...
treeSize :: Tree a -> Int
treeSize Empty= 0
treeSize (Node a son1 son2) = 1+ treeSize son1 + treeSize son2
...
treeMax Empty= 0
treeMax (Node a son1 son2)=max (max a (treeMax son1)) (max a (treeMax son2))
...
allValues condition (Node a son1 son2) = (condition a && allValues condition son1) && allValues condition son2
...
mapTree f (Node a son1 son2)=Node (f a) (mapTree  f son1) (mapTree  f son2)
...
cull val (Node a son1 son2)  
 | val == a =Empty 
 | otherwise = (Node a (cull val son1) (cull val son2))
------------------------------------------------------------------------------
...
isOrdered (Node a son1 son2)
  | (son1==Empty)&&(son2==Empty) = True
  | son2==Empty = isOrdered son1 && a> val_son1
  | son1==Empty = isOrdered son2 && a< val_son2
  | otherwise = isOrdered son1&&isOrdered son2 &&((a<val_son2)&& (a>val_son1))
    where val_son1=fromJust(valAtRoot son1) 
          val_son2=fromJust(valAtRoot son2)
------------------------------------------------------------------------------
...
walk [] (Node a son1 son2) = Just a
walk steps Empty =Nothing
walk (x:xs) (Node a son1 son2)
 | x==StepL = walk xs son1
 | otherwise= walk xs son2
------------------------------------------------------------------------------
...
set [] val (Node pre_val son1 son2) = Node val son1 son2
set path val Empty = Empty
set (x:xs) val (Node a son1 son2)
 | x== StepL =Node a ( set xs val son1) son2
 |otherwise = Node a son1( set xs val son2) 
...
search val (Node a son1 son2)
 | a==val =Just []
 | search val son1/=Nothing = Just (StepL:fromJust(search val son1)) 
 | search val son2/=Nothing = Just (StepR:fromJust(search val son2)) 
 |otherwise = Nothing
