...
buildList x 0 initial= [initial]
buildList x n initial= x: buildList x (n-1) initial 
...
recursivefunc 1 =1
recursivefunc x = recursivefunc(x-1)+x
sums :: Int -> [Int]
sums 1 = [1]
sums end=[recursivefunc x| x<-[1..end]] 
...
indexDefault [x] i def = if i> 0 || i<0
                          then def
			  else x
indexDefault (x:xs) i def  
  | i==0       =x
  | i>=1       = indexDefault xs (i-1) def
  | otherwise  = def
------------------------------------------------------------------------------
