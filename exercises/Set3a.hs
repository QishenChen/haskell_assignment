...
firstHalf = take . div . length $2

palindrome =  Eq reverse 
...
capitalize s= unwords . zipwith (++) (map toUpper . head $ words s) (map tail $ words s)
...
powers :: Int -> Int -> [Int]
powers k max = iterate (*k) (log max k) 1
...
joinToLength l s= filter(\x -> (length x)==l)[x++y| x<- s, y<-s]
...
sumRights s = sum . filter (\x -> type x == Int) s
