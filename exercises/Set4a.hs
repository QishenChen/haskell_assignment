...
import Data.Array
import qualified Data.Text.Array as Data
...
distinct []= True
distinct (x:xs)=foldr(&&) True (map(/=x) xs) && distinct xs 
--minelement :: Ord a => [a]->a
--minelement xs=foldr (\x y -> if x<y then x else y) (head xs)  xs 
------------------------------------------------------------------------------
...
longest :: Ord a=>[[a]]->[a]
longest xs = foldr (compare_len) [] xs
             where compare_len x y
	            | length x> length y = x
		    | length x<length y =y
...
incrementKey fir arr= map add_one arr
                      where add_one x = if fst x == fir then (fst x, snd x+1) else x
...
average xs = getDiv (foldr (+) 0 xs) (length xs)  
             where getDiv x y = x / fromIntegral y
...
freqs = foldr f Map.empty 
            where f thismap = Map.alter g thismap
...
transfer from to amount bank 
 ...
 | otherwise =Map.alter add_amount to (Map.alter sub_amount from bank) 
              where add_amount x = Just(fromJust x+amount)
	            sub_amount x = Just(fromJust x-amount)
...
maxIndex arr= foldr compareindexarr head_arr_index arr_index 
            where  compareindexarr x y = if arr!x>arr!y then x else y 
	           head_arr_index=head (Data.Array.indices arr)
		   arr_index= Data.Array.indices arr
