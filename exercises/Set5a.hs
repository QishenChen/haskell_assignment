...
buyOneMore (MkShoppingEntry s a b)= MkShoppingEntry s a (b+1)
...
getName (Constructperson age name) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (Constructperson age name) = age
-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName new_name  (Constructperson age name)= Constructperson age new_name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge new_age (Constructperson age name)= Constructperson new_age name
------------------------------------------------------------------------------
...
data Position =SetPosition Int Int
...
data UpDown = ConstructIncrease Int | Constructdecrease Int

-- zero is an increasing counter with value 0
zero :: UpDown
zero = ConstructIncrease 0

-- get returns the counter value
get :: UpDown -> Int
get (ConstructIncrease upv) = upv 
get (Constructdecrease downv) = downv 

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (ConstructIncrease upv) = ConstructIncrease (upv+1)
tick (Constructdecrease v) = Constructdecrease (v-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown

toggle (ConstructIncrease upv) = Constructdecrease (upv)
toggle (Constructdecrease v) = ConstructIncrease  v
------------------------------------------------------------------------------
...
rgb Green = [0,1,0]
rgb Blue  = [0,0,1]
rgb (Mix color1 color2)= [(rgbcol1!!0+rgbcol2!!0)/2,(rgbcol1!!1+rgbcol2!!1)/2,(rgbcol1!!2+rgbcol2!!2)/2]
  where rgbcol1= rgb color1
        rgbcol2= rgb color2
rgb (Invert color)=[1-rgbcol!!0,1-rgbcol!!1, 1-rgbcol!!2]
  where rgbcol= rgb color
------------------------------------------------------------------------------
...
fromList []=Empty
fromList xs = foldr (\x rest->Pair (fst x) (snd x) rest) Empty xs
------------------------------------------------------------------------------
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne nat_num) = 1+fromNat nat_num

toNat :: Int -> Maybe Nat
toNat int_num 
 | int_num < 0 = Nothing
 | int_num ==0 = Just Zero
 | otherwise = Just (PlusOne lessNat)
    where lessNat=  fromJust(toNat (int_num-1))
------------------------------------------------------------------------------
...
toBin 0= O End
toBin 1 = I End
toBin n= if mod n 2 ==0 then (O (toBin (div n 2))) else (I (toBin (div n 2)))
